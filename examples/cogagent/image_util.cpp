...
bool clip_image_load_from_bytes(const unsigned char * bytes, size_t bytes_length, struct clip_image_u8 * img) {
    ...
    if (!data) {
        printf("%s: failed to decode image bytes\n", __func__);
        return false;
    }
    build_clip_img_from_data(data, nx, ny, img);
    stbi_image_free(data);
...
static bool load_file_to_bytes(const char* path, unsigned char** bytesOut, long *sizeOut) {
    ...
    if (buffer == NULL) {
        printf("%s: failed to alloc %ld bytes for file %s\n", __func__, fileSize, path);
...
// The above is copied from llava.
// The following is added for resizing an image
...
bool load_and_stretch_image(const char* path, int output_size, std::vector<float> &output_data) {
    ...
    if (!clip_image_load_from_bytes(image_bytes, image_size, clip_img)) {
        clip_image_u8_free(clip_img);
        free(image_bytes);
        printf("Failed to create CLIP image structure from image bytes.\n");
        return false;
    }
    ...
    for (int c=0; c<3; c++) {
        for (int y=0; y<output_size; y++) {
            for (int x=0; x<output_size; x++) {
                // The data is for a tensor with shape h/w, h/w, 3, b
